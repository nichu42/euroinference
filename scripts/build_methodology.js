...
const sourcePath = path.join(rootDir, 'METHODOLOGY.md');
const outputPath = path.join(rootDir, 'methodology.html');
...
function renderMarkdown(markdown) {
  ...
  const title = titleIndex >= 0 ? lines[titleIndex].slice(2).trim() : 'Cost & Caching Methodology';
  const reviewedLine = lines.find(line => /^\*\*Last reviewed:\*\*/.test(line));
  const reviewedDate = reviewedLine
    ? reviewedLine.replace(/^\*\*Last reviewed:\*\*\s*/, '').trim()
    : '';
  const bodyLines = lines.filter((line, index) => index !== titleIndex && line !== reviewedLine);
  const output = [];
  let paragraph = [];
  let inList = false;
...
function buildPage({ title, effectiveDate, body }) {
  ...
  const escapedDate = effectiveDate ? `Last reviewed: ${escapeHtml(effectiveDate)}` : '';
  return `<!--
  Generated from METHODOLOGY.md by scripts/build_methodology.js.
  ...
  <div class="container policy-container">
    <header class="policy-header">
      <div class="policy-title">
        ...
        <div>
          <p class="policy-kicker">Transparency</p>
          <h1>${escapedTitle}</h1>
