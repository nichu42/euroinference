...
Each offer shows `✓ EU`, `EU (US)`, `EU-Sovereign` or `Global` for jurisdiction, plus retention, training, and where the model runs. `? Unknown` means the provider doesn't say — we don't guess. Details come from the provider's own API signals or `config/sovereignty.json` (public, source-linked facts).
